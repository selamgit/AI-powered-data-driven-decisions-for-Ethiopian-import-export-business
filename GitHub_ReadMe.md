...
Currently, Ethiopia is one of the top fastest growing economies in Africa, and
...
\#url =
'https://github.com/selamgit/AI-powered-data-driven-decisions-for-Ethiopian-import-export-business/upload/master/import_2017_2.csv'
...
\#df =
pd.read_csv(io.StringIO(uploaded['import_2017_2.csv'].decode('ISO-8859-1')))

\#df = df.rename(columns={c: c.replace(' ', '') for c in df.columns}) \# Remove
spaces from columns
...
others1 = (total_import.CIFValueUSD).sum() - (small_import.CIFValueUSD).sum()

long_df = small_import.reset_index()

long_df.loc[-1] = ['Other '+str(rows)+' countries' , others1]
...
country_share_group = country_share_value.groupby('CountryOrigin',as_index =
False)
...
big_share = total_country_share[total_country_share.CIFValueUSD \>
300000000].dropna()

small_share = total_country_share[total_country_share.CIFValueUSD \<
300000000].dropna()

others_share = (total_country_share.CIFValueUSD).sum() -
(big_share.CIFValueUSD).sum()

country_list = big_share["CountryOrigin"].unique()

fracs = list(big_share.CIFValueUSD)

fracs.append(others_share)
...
lst_number = len(C_ClfGtLabels) \# get length of the list
...
ax1.pie(fracs, labels=C_ClfGtLabels, explode=explod,
